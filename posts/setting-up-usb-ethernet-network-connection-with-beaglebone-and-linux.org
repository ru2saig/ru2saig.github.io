...
Take a note of either of the interface names[fn:1] and the corresponding ip. I will be going with the interface enp0s210f0u2i2 that has the ip 192.168.6.1, unless otherwise stated. Some of the sections have commands run for both of the interfaces, but solely using one interface will also work. 
...
#+BEGIN_SRC sh
  ...
  root$ ufw route allow in on enp0s20f0u2 out on wlp3s0 # enp0s20f0u2 is the other interface of the bone
